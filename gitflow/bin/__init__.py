...
class SupportCommand(GitFlowCommand):
    ...
    def run_start(args):
        ...
        # NB: `args.name` is required since the branch must not yet exist
        # :fixme: get default value for `base`
        gitflow.start_transaction('create support branch %s (from %s)' % (args.name, args.base))
        try:
...
if __name__ == '__main__':
    try:
        main()
    except (GitflowError, GitCommandError) as e:
